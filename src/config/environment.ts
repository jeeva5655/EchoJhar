...
export const config = {
  ...
  // API endpoints
  apiEndpoints: {
    // EchoJhar Backend API (deployed on Render)
    backend: getEnvVar('VITE_API_URL') || 'https://echojhar.onrender.com',
...
// Backend API helpers
export const getBackendApiUrl = (): string => config.apiEndpoints.backend;
export const getApiUrl = (endpoint: string): string => `${config.apiEndpoints.backend}/api${endpoint}`;
